...
{
    public class DAL_Atto
    {
        string strConexao;
        SqlConnection SQLConexao;
        static DAL_Atto instancia;

        public DAL_Atto()
        {
            /* BASE DESENVOLVIMENTO */
            strConexao = "Server=172.20.30.12; Database=CALLTABLES; User Id=mis; Password=41l154@c; Max Pool Size=600";
            SQLConexao = new SqlConnection(strConexao);
        }

        public static DAL_Atto Instancia
        {
            get
            {
                if (instancia == null)
                    instancia = new DAL_Atto();
                return instancia;
            }
        }
        ...
        public int ExecutaComandoSQL(SqlCommand sqlCommand)
        {
            ...
            catch (Exception ex)
            {
                sqltran.Rollback();
                DesconectaDataBase();
                throw new Exception("DAL_Atto_N_001: " + ex.Message, ex);
            }
        }
        ...
        public DataSet ConsultaSQL(SqlCommand sqlCommand)
        {
            ...
            catch (Exception ex)
            {
                sqltran.Rollback();
                DesconectaDataBase();
                throw new Exception("DAL_Atto_N_002: " + ex.Message, ex);
            }
        }
        ...
        public int ExecutaBulkCopySQL(DataTable TabelaCarga, string TabelaDestino)
        {
            ...
            {
                DesconectaDataBase();
                throw new Exception("DAL_Atto_N_003: " + ex.Message, ex);
            }
        }
        public int ExecutaBulkCopySQL(DataRow[] TabelaCarga, string TabelaDestino)
        {
            ...
            {
                DesconectaDataBase();
                throw new Exception("DAL_Atto_N_004: " + ex.Message, ex);
            }
        }
    }
}