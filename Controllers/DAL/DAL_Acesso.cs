...
namespace Intranet_NEW.Controllers.DAL
{
    public class DAL_Acesso
    {
        ...
        static DAL_Acesso instancia;

        public DAL_Acesso()
        {
            /* BASE DESENVOLVIMENTO */
            strConexao = "Server=172.20.1.248; Database=RBACESSO_V100; User Id=sa; Password=t!gt3@br@sil; Max Pool Size=600";
            SQLConexao = new SqlConnection(strConexao);
        }

        public static DAL_Acesso Instancia
        {
            get
            {
                if (instancia == null)
                    instancia = new DAL_Acesso();
                return instancia;
            }
        }
        ...
        public int ExecutaComandoSQL(SqlCommand sqlCommand)
        {
            ...
            catch (Exception ex)
            {
                sqltran.Rollback();
                DesconectaDataBase();
                throw new Exception("DAL_Acesso_N_001: " + ex.Message, ex);
            }
        }
        ...
        public DataSet ConsultaSQL(SqlCommand sqlCommand)
        {
            ...
            catch (Exception ex)
            {
                sqltran.Rollback();
                DesconectaDataBase();
                throw new Exception("DAL_Acesso_N_002: " + ex.Message, ex);
            }
        }
        ...
        public int ExecutaBulkCopySQL(DataTable TabelaCarga, string TabelaDestino)
        {
            ...
            {
                DesconectaDataBase();
                throw new Exception("DAL_Acesso_N_003: " + ex.Message, ex);
            }
        }
        public int ExecutaBulkCopySQL(DataRow[] TabelaCarga, string TabelaDestino)
        {
            ...
            {
                DesconectaDataBase();
                throw new Exception("DAL_Acesso_N_004: " + ex.Message, ex);
            }
        }
    }
}
