...
{
    public class DAL_Easycollector
    {
        string strConexao;
        SqlConnection SQLConexao;
        static DAL_Easycollector instancia;

        public DAL_Easycollector()
        {
            /* BASE DESENVOLVIMENTO */
            strConexao = "Server=172.20.1.17;Database=easycollector;User Id=consultamarcos;Password=c12345@@;";
            SQLConexao = new SqlConnection(strConexao);
        }

        public static DAL_Easycollector Instancia
        {
            get
            {
                if (instancia == null)
                    instancia = new DAL_Easycollector();
                return instancia;
            }
        }
