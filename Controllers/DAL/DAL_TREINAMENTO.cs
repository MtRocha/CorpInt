...
{
    public class DAL_TREINAMENTO
    {
        string strConexao;
        SqlConnection SQLConexao;
        static DAL_TREINAMENTO instancia;

        public DAL_TREINAMENTO()
        {
            /* BASE DESENVOLVIMENTO */
            strConexao = "Server=172.20.1.248; Database=DB_TREINAMENTO; User Id=SisIntranet; Password=_P@ssw0rdEX!T0; Max Pool Size=600";
            SQLConexao = new SqlConnection(strConexao);
        }

        public static DAL_TREINAMENTO Instancia
        {
            get
            {
                if (instancia == null)
                    instancia = new DAL_TREINAMENTO();
                return instancia;
            }
        }
        ...
        public int ExecutaComandoSQL(SqlCommand sqlCommand)
        {
            ...
            catch (Exception ex)
            {
                sqltran.Rollback();
                DesconectaDataBase();
                throw new Exception("DAL_TREINAMENTO_001: " + ex.Message, ex);
            }
        }
        ...
        public DataSet ConsultaSQL(SqlCommand sqlCommand)
        {
            ...
            catch (Exception ex)
            {
                sqltran.Rollback();
                DesconectaDataBase();
                throw new Exception("DAL_TREINAMENTO_002: " + ex.Message, ex);
            }
        }
        ...
        public int ExecutaBulkCopySQL(DataTable TabelaCarga, string TabelaDestino)
        {
            ...
            {
                DesconectaDataBase();
                throw new Exception("DAL_TREINAMENTO_003: " + ex.Message, ex);
            }
        }
        public int ExecutaBulkCopySQL(DataRow[] TabelaCarga, string TabelaDestino)
        {
            ...
            {
                DesconectaDataBase();
                throw new Exception("DAL_TREINAMENTO_004: " + ex.Message, ex);
            }
        }
    }
}
