...
namespace Intranet_NEW.DAL
{
    ...
    public class DAL_INTRANET
    {
        string strConexao;
        SqlConnection SQLConexao;
        static DAL_INTRANET instancia;

        public DAL_INTRANET()
        {
            /* BASE DESENVOLVIMENTO */
            strConexao = "Server=172.20.1.248; Database=DB_INTRANET;User Id=SisIntranet; Password=_P@ssw0rdEX!T0; TrustServerCertificate=True;";
            SQLConexao = new SqlConnection(strConexao);
        }

        public static DAL_INTRANET Instancia
        {
            get
            {
                if (instancia == null)
                    instancia = new DAL_INTRANET();
                return instancia;
            }
        }
        ...
        public int ExecutaComandoSQL(SqlCommand sqlCommand)
        {
            ...
            catch (Exception ex)
            {
                sqltran.Rollback();
                DesconectaDataBase();
                throw new Exception("DAL_INTRANET_N_001: " + ex.Message, ex);
            }
        }
        ...
        public DataSet ConsultaSQL(SqlCommand sqlCommand)
        {
            ...
            catch (Exception ex)
            {
                sqltran.Rollback();
                DesconectaDataBase();
                throw new Exception("DAL_INTRANET_N_002: " + ex.Message, ex);
            }
        }
        ...
        public int ExecutaBulkCopySQL(DataTable TabelaCarga, string TabelaDestino)
        {
            ...
            {
                DesconectaDataBase();
                throw new Exception("DAL_INTRANET_N_003: " + ex.Message, ex);
            }
        }
        public int ExecutaBulkCopySQL(DataRow[] TabelaCarga, string TabelaDestino)
        {
            ...
            {
                DesconectaDataBase();
                throw new Exception("DAL_INTRANET_N_004: " + ex.Message, ex);
            }
        }
    }
}
